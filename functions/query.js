export async function onRequestGet({ request, env }) {
  ...
  const result = await env.DB.prepare("SELECT v1.bill_id, v1.desc, v1.date, v1.yea_votes, v1.nay_votes, v1.chamber, m.measure_number FROM votes_1 AS v1 WHERE roll_call_id = ? INNER JOIN measures AS m ON v1.bill_id = m.bill_id;")
  .bind(id)
  .all();
