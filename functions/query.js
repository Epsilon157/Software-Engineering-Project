export async function onRequestGet({ request, env }) {
  ...
  const result = await env.DB.prepare("SELECT v1.bill_id, v1.desc, v1.date, v1.yea_votes, v1.nay_votes, v1.chamber, m.measure_number, m.measure_type, m.primary_author, c.people_id AS coauthor_id FROM votes_1 AS v1 INNER JOIN measures AS m ON v1.bill_id = m.bill_id INNER JOIN coauthors AS c ON v1.bill_id = c.bill_id WHERE v1.roll_call_id = ?;")
  .bind(id)
  .all();
